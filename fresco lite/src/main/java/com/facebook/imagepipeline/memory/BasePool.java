...
public abstract class BasePool<V> implements Pool<V>
{
    ...
    public V get (int size)
    {
        ...
        synchronized (this)
        {
            ...
            if (bucket != null)
            {
                ...
                if (value != null)
                {
                    ...
                    {
                        FLog.v(TAG, "get (reuse) (object, size) = (%x, %s)", System.identityHashCode(value), bucketedSize);
                    }
                    return value;
                }
                // fall through
            ...
            {
                throw new PoolSizeViolationException(mPoolParams.maxSizeHardCap, mUsed.mNumBytes, mFree.mNumBytes, sizeInBytes);
            }
        ...
        synchronized (this)
        {
            ...
            if (FLog.isLoggable(FLog.VERBOSE))
            {
                FLog.v(TAG, "get (alloc) (object, size) = (%x, %s)", System.identityHashCode(value), bucketedSize);
            }
        }
    ...
    public void release (V value)
    {
        ...
        synchronized (this)
        {
            ...
            if (!mInUseValues.remove(value))
            {
                ...
                FLog.e(TAG, "release (free, value unrecognized) (object, size) = (%x, %s)", System.identityHashCode(value), bucketedSize);
                free(value);
            ...
            else
            {
                        ...
                        !isReusable(value))
                {
                    ...
                    if (FLog.isLoggable(FLog.VERBOSE))
                    {
                        FLog.v(TAG, "release (free) (object, size) = (%x, %s)", System.identityHashCode(value), bucketedSize);
                    }
                    free(value);
                    mUsed.decrement(sizeInBytes);
                    mPoolStatsTracker.onFree(sizeInBytes);
                ...
                else
                {
                    ...
                    if (FLog.isLoggable(FLog.VERBOSE))
                    {
                        FLog.v(TAG, "release (reuse) (object, size) = (%x, %s)", System.identityHashCode(value), bucketedSize);
                    }
                }
            }
            logStats();
    ...
    private void logStats ()
    {
        if (FLog.isLoggable(FLog.VERBOSE))
        {
            FLog.v(TAG, "Used = (%d, %d); Free = (%d, %d)", mUsed.mCount, mUsed.mNumBytes, mFree.mCount, mFree.mNumBytes);
        }
    }
    ...
    public synchronized Map<String, Integer> getStats ()
    {
        ...
        for (int i = 0; i < mBuckets.size(); ++i)
        {
            ...
            final Bucket<V> bucket = mBuckets.valueAt(i);
            final String BUCKET_USED_KEY = PoolStatsTracker.BUCKETS_USED_PREFIX + getSizeInBytes(bucketedSize);
    ...
    static class Counter
    {
        ...
        public void decrement (int numBytes)
        {
            ...
            {
                FLog.wtf(TAG, "Unexpected decrement of %d. Current numBytes = %d, count = %d", numBytes, this.mNumBytes, this.mCount);
            }
        }
