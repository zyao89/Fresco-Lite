...
public class NativeMemoryChunk implements Closeable
{
     ...
     */
    public synchronized int write (int nativeMemoryOffset, final byte[] byteArray, int byteArrayOffset, int count)
    {
        ...
        nativeCopyFromByteArray(mNativePtr + nativeMemoryOffset, byteArray, byteArrayOffset, actualCount);
        return actualCount;
    }
     ...
     */
    public synchronized int read (final int nativeMemoryOffset, final byte[] byteArray, final int byteArrayOffset, final int count)
    {
        Preconditions.checkNotNull(byteArray);
        Preconditions.checkState(!isClosed());
        final int actualCount = adjustByteCount(nativeMemoryOffset, count);
     ...
     * @param count       number of bytes to copy
     */
    public void copy (final int offset, final NativeMemoryChunk other, final int otherOffset, final int count)
    {
        ...
        if (other.mNativePtr == mNativePtr)
        {
            ...
            Log.w(TAG, "Copying from NativeMemoryChunk " +
     ...
     * other objects
     */
    private void doCopy (final int offset, final NativeMemoryChunk other, final int otherOffset, final int count)
    {
        Preconditions.checkState(!isClosed());
     ...
     */
    private void checkBounds (final int myOffset, final int otherLength, final int otherOffset, final int count)
    {
