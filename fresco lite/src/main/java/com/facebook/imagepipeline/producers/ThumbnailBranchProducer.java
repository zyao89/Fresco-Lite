...
public class ThumbnailBranchProducer implements Producer<EncodedImage>
{
    ...
    private boolean produceResultsFromThumbnailProducer (int startIndex, Consumer<EncodedImage> consumer, ProducerContext context)
    {
        ...
        mThumbnailProducers[producerIndex].produceResults(new ThumbnailConsumer(consumer, context, producerIndex), context);
        return true;
    }
    ...
    private class ThumbnailConsumer extends DelegatingConsumer<EncodedImage, EncodedImage>
    {
        ...
        {
            if (newResult != null && (!isLast || ThumbnailSizeChecker.isImageBigEnough(newResult, mResizeOptions)))
            {
            ...
            else if (isLast)
            {
                ...
                boolean fallback = produceResultsFromThumbnailProducer(mProducerIndex + 1, getConsumer(), mProducerContext);
