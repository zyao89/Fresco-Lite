    ...
    private class PostprocessorConsumer extends DelegatingConsumer<CloseableReference<CloseableImage>, CloseableReference<CloseableImage>>
    {
        ...
        private void doPostprocessing (CloseableReference<CloseableImage> sourceImageRef, boolean isLast)
        {
            ...
            try
            {
                ...
                {
                    mListener.onProducerFinishWithFailure(mRequestId, NAME, e, getExtraMap(mListener, mRequestId, mPostprocessor));
                    maybeNotifyOnFailure(e);
                    return;
                }
                mListener.onProducerFinishWithSuccess(mRequestId, NAME, getExtraMap(mListener, mRequestId, mPostprocessor));
        ...
        private CloseableReference<CloseableImage> postprocessInternal (CloseableImage sourceImage)
        {
            ...
            try
            {
                return CloseableReference.<CloseableImage>of(new CloseableStaticBitmap(bitmapRef, sourceImage.getQualityInfo(), rotationAngle));
            }
            finally
            {
