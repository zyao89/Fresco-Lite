...
public class RequestLoggingListener implements RequestListener
{
    ...
    public synchronized void onRequestStart (ImageRequest request, Object callerContextObject, String requestId, boolean isPrefetch)
    {
        ...
        {
            FLog.v(TAG, "time %d: onRequestSubmit: {requestId: %s, callerContext: %s, isPrefetch: %b}", getTime(), requestId, callerContextObject, isPrefetch);
    ...
    public synchronized void onProducerStart (String requestId, String producerName)
    {
        if (FLog.isLoggable(FLog.VERBOSE))
        {
            ...
            FLog.v(TAG, "time %d: onProducerStart: {requestId: %s, producer: %s}", startTime, requestId, producerName);
        }
    }
    ...
    public synchronized void onProducerFinishWithFailure (String requestId, String producerName, Throwable throwable, @Nullable Map<String, String> extraMap)
    {
        if (FLog.isLoggable(FLog.WARN))
        {
            ...
            Long startTime = mProducerStartTimeMap.remove(mapKey);
            long currentTime = getTime();
            FLog.w(TAG, "time %d: onProducerFinishWithFailure: " + "{requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s, throwable: %s}", currentTime, requestId, producerName, getElapsedTime(startTime, currentTime), extraMap, throwable.toString());
        }
    }
    ...
    public synchronized void onProducerFinishWithCancellation (String requestId, String producerName, @Nullable Map<String, String> extraMap)
    {
        if (FLog.isLoggable(FLog.VERBOSE))
        {
            ...
            Long startTime = mProducerStartTimeMap.remove(mapKey);
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onProducerFinishWithCancellation: " + "{requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s}", currentTime, requestId, producerName, getElapsedTime(startTime, currentTime), extraMap);
        }
    }
