...
public class RoundedCornersDrawable extends ForwardingDrawable implements Rounded
{
            ...
            case OVERLAY_COLOR:
                ...
                if (mIsCircle)
                {
                    ...
                    if (paddingH > 0)
                    {
                        ...
                        canvas.drawRect(bounds.right - paddingH, bounds.top, bounds.right, bounds.bottom, mPaint);
                    }
                    if (paddingV > 0)
                    {
                        canvas.drawRect(bounds.left, bounds.top, bounds.right, bounds.top + paddingV, mPaint);
                        canvas.drawRect(bounds.left, bounds.bottom - paddingV, bounds.right, bounds.bottom, mPaint);
                    }
                }
                break;
        }
    ...
    private void updatePath ()
    {
        ...
        if (mIsCircle)
        {
            mPath.addCircle(mTempRectangle.centerX(), mTempRectangle.centerY(), Math.min(mTempRectangle.width(), mTempRectangle.height()) / 2, Path.Direction.CW);
        }
        else
        {
        ...
        if (mIsCircle)
        {
            ...
            mBorderPath.addCircle(mTempRectangle.centerX(), mTempRectangle.centerY(), radius, Path.Direction.CW);
        }
        else
        {
