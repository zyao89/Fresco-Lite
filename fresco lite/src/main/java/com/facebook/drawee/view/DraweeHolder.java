...
public class DraweeHolder<DH extends DraweeHierarchy> implements VisibilityCallback, MemoryUiTrimmable
{
    ...
    public void onDraw ()
    {
        ...
        if (!mTrimmed)
        {
            ...
            FLog.wtf(DraweeEventTracker.class, "%x: Draw requested for a non-attached controller %x. %s", System.identityHashCode(this), System.identityHashCode(mController), toString());
        }
        mTrimmed = false;
        mIsHolderAttached = true;
        mIsVisible = true;
    ...
    {
        return Objects.toStringHelper(this).add("controllerAttached", mIsControllerAttached).add("holderAttached", mIsHolderAttached).add("drawableVisible", mIsVisible).add("trimmed", mTrimmed).add("events", mEventTracker.toString()).toString();
    }
}
